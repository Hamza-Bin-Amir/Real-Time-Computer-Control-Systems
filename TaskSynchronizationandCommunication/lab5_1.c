#include <stdio.h>
#include <stdlib.h>
#include <sys/stat.h>
#include <string.h>
#include <pthread.h>
#include <semaphore.h>
#include <mqueue.h>
#include <errno.h>

struct Msg {char str[20]; unsigned int priority; };
int errno;

const char *qname = "/myque";  

void *Receiver(void *arg)  
{
   mqd_t rx;
   struct mq_attr my_attrs;
   ssize_t n;
   int nMsgs;
   char inBuf[50];
   unsigned int priority;

   sleep(2);

   rx = mq_open(qname, O_RDONLY);  

   if (rx < 0 ) perror("Receiver mq_open:");  

   mq_getattr(rx, &my_attrs);     
   nMsgs = my_attrs.mq_curmsgs;

   printf("Receiver found %d messages\n", nMsgs);

   while (nMsgs-- > 0) {
      n = mq_receive(rx, inBuf, sizeof(inBuf), &priority);
      inBuf[n] = '\0';
      if (n < 0) perror("Receiver mq_receive:");  
      printf("Receiver got <%s>, %d bytes at priority %d\n", inBuf, n, priority);
   }

   mq_close(rx);   
   pthread_exit(NULL);
}
   
void *Sender(void *arg)  
{
   int retcode, i;
   mqd_t tx;
   struct Msg myMsg;
   unsigned int msg_priority[5] = {3, 1, 5, 8, 4};  

   sleep(2);

   tx = mq_open(qname, O_WRONLY);  

   if (tx < 0) perror("Sender mq_open:");  

   for (i = 1; i <= 5; i++) {
      sprintf(myMsg.str, "Test msg %d", i);  
      myMsg.priority = msg_priority[i-1];    
      retcode = mq_send(tx, myMsg.str, (size_t)strlen(myMsg.str), myMsg.priority);
      if (retcode < 0) perror("Sender mq_send:");  
   }

   mq_close(tx);   
   pthread_exit(NULL);
}

int main(void *arg)
{
   mqd_t trx;
   mode_t mode;
   int oflags;
   struct mq_attr my_attrs;
   pthread_t Sender_thread, Receiver_thread;

   my_attrs.mq_maxmsg = 10;      
   my_attrs.mq_msgsize = 50;     

   oflags = O_CREAT | O_RDWR ;   
   
   mode = S_IRUSR | S_IWUSR;     

   trx = mq_open(qname, oflags, mode, &my_attrs);  

   if (trx < 0) perror("Main mq_open:");  

   printf("Creating Sender thread\n");
   if (pthread_create(&Sender_thread, NULL, &Sender, NULL) != 0) {
       printf("Error creating Sender thread.\n");
       exit(-1);
   }

   pthread_join(Sender_thread, NULL);  // Wait for sender thread to finish

   printf("Creating Receiver thread\n");
   if (pthread_create(&Receiver_thread, NULL, &Receiver, NULL) != 0) {
       printf("Error creating Receiver thread.\n");
       exit(-1);
   }

   pthread_join(Receiver_thread, NULL); // Wait for receiver thread to finish

   mq_unlink(qname);     
   exit(0);
}
